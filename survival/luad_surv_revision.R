...
# source("/share/data6/tmp/renjun/CellTrekResult/CellTrek/celltrek_code_all/survival/luad_surv_revision.R")
library(survival)
library(survminer)
library(philentropy)
library(forestplot)

xtcga = data.table::fread("./5_combine_plot/survival/TCGA.LUAD.sampleMap_HiSeqV2_PANCAN.gz", data.table = F)
rownames(xtcga) = xtcga[,1]
xtcga = xtcga[,-1]
xclin = read.table("./5_combine_plot/survival/survival_LUAD_survival.txt", sep = "\t", header = T)
...
load(paste0("/share/data6/tmp/renjun/ExtraJobs/viper/1_tcga_rna_data/PanCanAtlas/LUAD/clinical.rda"))
...
ggsave(paste0("luad_trajectory_survival_revision.pdf"), p, width = 14, height = 5, limitsize = FALSE)
